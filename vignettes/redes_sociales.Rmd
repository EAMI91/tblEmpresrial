...
```{r}
load(here("data", "candidatos.rda"))
candidatos <- filter(candidatos, candidato=="Candidato 1")
...
graficando_nube <- function(DB){
  nube <- textplot_wordcloud(Nube, comparison = TRUE, max_words = 300, adjust = 0, 
                   rotation   = 0.1, random_order = FALSE,random_color = FALSE,
                   ordered_color = FALSE,
                   color = c("#cb2833", "#0f4c42")) + theme_minimal()
  return(nube)
}
